...
public class AuthenticationParser {
    ...
    public static String parseBearer(String auth) throws AuthenticationParseException {
        ...
        if(auth == null) throw new AuthenticationParseException("Missing authorization header.");

        if(!auth.startsWith(BEARER_PREFIX)) throw new AuthenticationParseException("Invalid Basic authorization header.");
    ...
    public static Credentials parseBasic(String auth) throws AuthenticationParseException {
        ...
        if(!auth.startsWith(BASIC_PREFIX)) throw new AuthenticationParseException("Invalid Bearer authorization header.");

        String encoded = auth.substring(BASIC_PREFIX.length());
        String[] decoded = new String(Base64.getDecoder().decode(encoded)).split(":", -1);

        if(decoded.length != 2) throw new AuthenticationParseException("Invalid Bearer authorization header.");
